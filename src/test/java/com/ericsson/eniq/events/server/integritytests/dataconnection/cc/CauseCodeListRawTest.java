...
import com.ericsson.eniq.events.server.test.queryresults.gsm.dataconnection.CauseCodeListResult;
 ...
 *
 */
public class CauseCodeListRawTest extends BaseDataIntegrityTest<CauseCodeListResult> {
    ...
    private SubscriberCCListService imsiService;
    ...
    @Test
    public void testIMSIFiveMinuteQueryForSuccessAndErrorEvents() throws URISyntaxException, Exception {
        insertSuccessEventData();
        insertErrorEventData();
        final String result = runIMSIFiveMinuteQuery();
        verifyResultForSuccessAndErrorEvents(result);
    }

    @Test
    public void testIMSIFiveMinuteQueryForSuccessOnlyEvents() throws URISyntaxException, Exception {
        insertSuccessEventData();
        final String result = runIMSIFiveMinuteQuery();
        verifyResultForSuccessOnlyEvents(result);
    }

    @Test
    public void testIMSIFiveMinuteQueryForErrorOnlyEvents() throws URISyntaxException, Exception {
        insertErrorEventData();
        final String result = runIMSIFiveMinuteQuery();
        verifyResultForErrorOnlyEvents(result);
    }

    @Test
    public void testIMSIGroupFiveMinuteQueryForSuccessAndErrorEvents() throws URISyntaxException, Exception {
        insertGroupData();
        insertSuccessEventData();
        insertErrorEventData();
        final String result = runIMSIGroupFiveMinuteQuery();
        verifyGroupResultForSuccessAndErrorEvents(result);
    }

    @Test
    public void testIMSIGroupFiveMinuteQueryForSuccessOnlyEvents() throws URISyntaxException, Exception {
        insertGroupData();
        insertSuccessEventData();
        final String result = runIMSIGroupFiveMinuteQuery();
        verifyResultForSuccessOnlyEvents(result);
    ...
    @Test
    public void testIMSIGroupFiveMinuteQueryForErrorOnlyEvents() throws URISyntaxException, Exception {
        insertGroupData();
        insertErrorEventData();
        final String result = runIMSIGroupFiveMinuteQuery();
        verifyGroupResultForErrorOnlyEvents(result);
    }

    private String runIMSIFiveMinuteQuery() {
        imsiService = new SubscriberCCListService();
    ...
    private String runIMSIGroupFiveMinuteQuery() {
        imsiService = new SubscriberCCListService();
        attachDependencies(imsiService);
        final MultivaluedMap<String, String> requestParameters = new MultivaluedMapImpl();
        requestParameters.add(TZ_OFFSET, TZ_OFFSET_OF_ZERO);
    ...
    private void verifyResultForSuccessAndErrorEvents(final String json) throws Exception {
        assertJSONSucceeds(json);
        final List<CauseCodeListResult> results = getTranslator().translateResult(json, CauseCodeListResult.class);

        assertThat(results.size(), is(4));//success included, and 3 other cause codes
        ...
        assertThat(results.get(1).getCauseCodeId(), is(TEST_VALUE_TBF_RELEASE_CAUSE_5));
        assertThat(results.get(1).getCauseCodeDesc(), is(TEST_VALUE_TBF_RELEASE_CAUSE_5_DESC));

        assertThat(results.get(2).getCauseCodeId(), is(TEST_VALUE_TBF_RELEASE_CAUSE_6));
        assertThat(results.get(2).getCauseCodeDesc(), is(TEST_VALUE_TBF_RELEASE_CAUSE_6_DESC));

        assertThat(results.get(3).getCauseCodeId(), is(DATA_CONNECTION_SUCCESS_CAUSE_CODE_ID_LABEL));
        assertThat(results.get(3).getCauseCodeDesc(), is(SUCCESSES));
    }

    private void verifyResultForSuccessOnlyEvents(final String json) throws Exception {
        assertJSONSucceeds(json);
        final List<CauseCodeListResult> results = getTranslator().translateResult(json, CauseCodeListResult.class);

        assertThat(results.size(), is(1));//success only included
    ...
    private void verifyResultForErrorOnlyEvents(final String json) throws Exception {
        assertJSONSucceeds(json);
        final List<CauseCodeListResult> results = getTranslator().translateResult(json, CauseCodeListResult.class);

        assertThat(results.size(), is(3));// 3 error cause codes
        assertThat(results.get(0).getCauseCodeId(), is(TEST_VALUE_TBF_RELEASE_CAUSE_0));
        assertThat(results.get(0).getCauseCodeDesc(), is(TEST_VALUE_TBF_RELEASE_CAUSE_0_DESC));
    ...
    private void verifyGroupResultForSuccessAndErrorEvents(final String json) throws Exception {
        assertJSONSucceeds(json);
        final List<CauseCodeListResult> results = getTranslator().translateResult(json, CauseCodeListResult.class);
    ...
    private void verifyGroupResultForErrorOnlyEvents(final String json) throws Exception {
        assertJSONSucceeds(json);
        final List<CauseCodeListResult> results = getTranslator().translateResult(json, CauseCodeListResult.class);
    ...
    private void insertSuccessEventData() throws Exception {
        ...
        insertData(true, TEST_VALUE_IMSI1, TEST_VALUE_TBF_RELEASE_CAUSE_0, dateTime2mins, 1, TEST_VALUE_TAC);
        insertData(true, TEST_VALUE_IMSI2, TEST_VALUE_TBF_RELEASE_CAUSE_0, dateTime2mins, 2, TEST_VALUE_TAC);
        insertData(true, TEST_VALUE_IMSI1, TEST_VALUE_TBF_RELEASE_CAUSE_0, dateTime3mins, 3, TEST_VALUE_TAC);
        insertData(true, TEST_VALUE_IMSI1, TEST_VALUE_TBF_RELEASE_CAUSE_0, dateTime48Hours, 3, TEST_VALUE_TAC);
        insertData(true, TEST_VALUE_IMSI1, TEST_VALUE_TBF_RELEASE_CAUSE_0, dateTime3mins, 3, SAMPLE_EXCLUSIVE_TAC);
        insertData(true, TEST_VALUE_IMSI1, TEST_VALUE_TBF_RELEASE_CAUSE_2, dateTime2mins, 1, TEST_VALUE_TAC);
        insertData(true, TEST_VALUE_IMSI1, TEST_VALUE_TBF_RELEASE_CAUSE_4, dateTime2mins, 1, TEST_VALUE_TAC);
    }

    private void insertErrorEventData() throws Exception {
        final String dateTime2mins = DateTimeUtilities.getDateTimeMinus2Minutes();
        final String dateTime3mins = DateTimeUtilities.getDateTimeMinus3Minutes();
        final String dateTime48Hours = DateTimeUtilities.getDateTimeMinus48Hours();
        //note - I have added cause code 0 as both successes and errors to mimic the case of where the white list changes
        insertData(false, TEST_VALUE_IMSI1, TEST_VALUE_TBF_RELEASE_CAUSE_0, dateTime2mins, 1, TEST_VALUE_TAC);//should be in result
        insertData(false, TEST_VALUE_IMSI3, TEST_VALUE_TBF_RELEASE_CAUSE_2, dateTime2mins, 2, TEST_VALUE_TAC);//not expected in result - different imsi
        insertData(false, TEST_VALUE_IMSI1, TEST_VALUE_TBF_RELEASE_CAUSE_0, dateTime3mins, 3, TEST_VALUE_TAC);//should be in result
        insertData(false, TEST_VALUE_IMSI2, TEST_VALUE_TBF_RELEASE_CAUSE_3, dateTime3mins, 3, TEST_VALUE_TAC);//should be in group result only
        insertData(false, TEST_VALUE_IMSI1, TEST_VALUE_TBF_RELEASE_CAUSE_4, dateTime48Hours, 9, TEST_VALUE_TAC); //not expected in result - outside time limit
        insertData(false, TEST_VALUE_IMSI1, TEST_VALUE_TBF_RELEASE_CAUSE_5, dateTime3mins, 3, SAMPLE_EXCLUSIVE_TAC);//should be in IMSI result, but NOT group result - tacs not excluded for explicit imsi request
        insertData(false, TEST_VALUE_IMSI1, TEST_VALUE_TBF_RELEASE_CAUSE_6, dateTime2mins, 1, TEST_VALUE_TAC);//should be in result
        insertData(false, TEST_VALUE_IMSI1, TEST_VALUE_TBF_RELEASE_CAUSE_7, dateTime2mins, 0, TEST_VALUE_TAC);//should be in result
    }
