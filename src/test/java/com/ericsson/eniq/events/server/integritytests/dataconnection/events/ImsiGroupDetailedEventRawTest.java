...
import com.ericsson.eniq.events.server.test.queryresults.gsm.dataconnection.SubscriberGroupDetailedEventAnalysisResult;
 ...
 *
 */
public class ImsiGroupDetailedEventRawTest extends BaseDataIntegrityTest<SubscriberGroupDetailedEventAnalysisResult> {

    private SubscriberGroupDetailedService service;

    @Before
    public void setup() throws Exception {
        service = new SubscriberGroupDetailedService();
    ...
    private void verifyResult(final String json) throws Exception {
        ...
        validateAgainstGridDefinition(json, "GSM_PS_SUBSCRIBER_DETAILED_EVENT_ANALYSIS_BY_IMSI_GROUP");
        final List<SubscriberGroupDetailedEventAnalysisResult> results = getTranslator().translateResult(json,
                SubscriberGroupDetailedEventAnalysisResult.class);

        assertThat(results.size(), is(5));
        int imsi1Counter = 0;
        int imsi2Counter = 0;
        for (int i = 0; i < results.size(); ++i) {
            final SubscriberGroupDetailedEventAnalysisResult result = results.get(i);
            assertThat(result.getEventType(), is(TEST_VALUE_CATEGORY_ID_DESC));
            assertThat(result.getAccessArea(), is(TEST_VALUE_GSM_CELL1_NAME));
            ...
            if (result.getIMSI().equals(TEST_VALUE_IMSI1)) {
                ++imsi1Counter;
            }
            if (result.getIMSI().equals(TEST_VALUE_IMSI2)) {
                ++imsi2Counter;
            }
        }
        assertThat("We have the required number of IMSI1s", imsi1Counter, is(3));
        assertThat("We have the required number of IMSI12s", imsi2Counter, is(2));
    }

    private void insertEventData(final String dateTime) throws Exception {
        insertData(TEST_VALUE_IMSI1, TEST_VALUE_TAC, dateTime, 3);
        insertData(TEST_VALUE_IMSI2, TEST_VALUE_TAC, dateTime, 2);
        insertData(TEST_VALUE_IMSI3, TEST_VALUE_TAC, dateTime, 3);
    ...
    private void insertData(final String imsi, final int tac, final String time, final int instances)
        ...
        for (int i = 0; i < instances; i++) {
            ...
            dataForEventTable.put(CHANNEL_RELATED_RELEASE_CAUSE_GROUP, TEST_VALUE_CHANNEL_RELATED_RELEASE_CAUSE_GROUP);
            ...
            dataForEventTable.put(GPRS_MEAS_REPORT_RXQUAL_DL, TEST_VALUE_GPRS_MEAS_REPORT_RXQUAL_DL);
            dataForEventTable.put(GPRS_MEAS_REPORT_SIGN_VAR, TEST_VALUE_GPRS_MEAS_REPORT_SIGN_VAR);
            dataForEventTable.put(GPRS_MEAS_REPORT_MEAN_BEP, TEST_VALUE_GPRS_MEAS_REPORT_MEAN_BEP);
            dataForEventTable.put(GPRS_MEAS_REPORT_CV_BEP, TEST_VALUE_GPRS_MEAS_REPORT_CV_BEP);
            dataForEventTable.put(GPRS_MEAS_REPORT_CVALUE, TEST_VALUE_GPRS_MEAS_REPORT_CVALUE);
