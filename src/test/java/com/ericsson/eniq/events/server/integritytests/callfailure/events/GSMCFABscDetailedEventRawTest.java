...
import com.ericsson.eniq.events.server.test.queryresults.gsm.callfailure.GSMCallFailureBscDetailedEventAnalysisResult;
 ...
 *
 */
public class GSMCFABscDetailedEventRawTest extends BaseDataIntegrityTest<GSMCallFailureBscDetailedEventAnalysisResult> {
    ...
    private static final String TEST_VALUE_HIER321_ID = "5386564559998864912";
    ...
    private ControllerDetailedService service;

    private long hashId;

    @Before
    public void setup() throws Exception {
        service = new ControllerDetailedService();
    ...
    public void testFiveMinuteQueryForHash() throws URISyntaxException, Exception {
        ...
        requestParameters.add(TZ_OFFSET, TZ_OFFSET_OF_ZERO);
        requestParameters.add(TIME_QUERY_PARAM, FIVE_MINUTES);
        requestParameters.add(CONTROLLER_SQL_ID, Long.toString(hashId));
        requestParameters.add(CATEGORY_ID, TEST_VALUE_CATEGORY_ID);
        final String result = runQuery(service, requestParameters);
        verifyResult(result);
    }
    ...
    private void verifyResult(final String json) throws Exception {
        ...
        validateAgainstGridDefinition(json, "NETWORK_RANKING_RAN_GSM_DETAILED_EVENT_ANALYSIS_BSC");
        final List<GSMCallFailureBscDetailedEventAnalysisResult> results = getTranslator().translateResult(json,
                GSMCallFailureBscDetailedEventAnalysisResult.class);

        assertThat(results.size(), is(1));
        final GSMCallFailureBscDetailedEventAnalysisResult result = results.get(0);
    ...
    private void insertData(final String imsi, final String eventID, final int tac, final String time,
    ...
    private void createLookupTables() throws Exception {
        ...
        createAndReplaceLookupTable(TEMP_DIM_E_GSM_CFA_RELEASE_TYPE, "DIM_E_GSM_CFA_RELEASE_TYPE", RELEASE_TYPE,
                RELEASE_TYPE_DESC);
        createAndReplaceLookupTable(TEMP_DIM_E_SGEH_HIER321, "DIM_E_SGEH_HIER321", HIERARCHY_1, HIER321_ID, HIER3_ID,
                RAT);
    }
