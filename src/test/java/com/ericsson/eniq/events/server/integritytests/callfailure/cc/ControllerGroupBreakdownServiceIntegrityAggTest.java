...
import com.ericsson.eniq.events.server.test.queryresults.gsm.callfailure.ControllerGroupBreakdownServiceRawResult;
import com.ericsson.eniq.events.server.test.util.DateTimeUtilities;
import com.sun.jersey.core.util.MultivaluedMapImpl;

public class ControllerGroupBreakdownServiceIntegrityAggTest extends
        BaseDataIntegrityTest<ControllerGroupBreakdownServiceRawResult> {
    ...
    private ControllerGroupBreakdownService service;

    @Before
    public void onSetUp() throws Exception {
        service = new ControllerGroupBreakdownService();
    ...
    private void verifyResult(final String json) throws Exception {
        ...
        final List<ControllerGroupBreakdownServiceRawResult> results = getTranslator().translateResult(json,
                ControllerGroupBreakdownServiceRawResult.class);

        assertThat(results.size(), is(2));
        final ControllerGroupBreakdownServiceRawResult firstResult = results.get(0);
        ...
        final ControllerGroupBreakdownServiceRawResult secondResult = results.get(1);
    ...
    private void insertRawEventData() throws Exception {
        ...
        insertData(TEST_VALUE_IMSI, TEST_VALUE_CATEGORY_ID, TEST_VALUE_HIER3_ID_1, TEST_VALUE_HIER321_ID_1,
                TEST_VALUE_TAC, dateTime, 3, TEST_VALUE_CAUSE_VALUE, TEST_VALUE_SUB_CAUSE_CODE); //1 impacted subscribers for HIER3_ID_1 as it is 3 instances for same imsi
        insertData(TEST_VALUE_IMSI, TEST_VALUE_CATEGORY_ID, TEST_VALUE_HIER3_ID_1, TEST_VALUE_HIER321_ID_2,
                TEST_VALUE_TAC, dateTime, 1, TEST_VALUE_CAUSE_VALUE, TEST_VALUE_SUB_CAUSE_CODE); //2 impacted cells for this BSC
        ...
        insertData(TEST_VALUE_IMSI_1, TEST_VALUE_CATEGORY_ID, TEST_VALUE_HIER3_ID_2, TEST_VALUE_HIER321_ID_1,
                TEST_VALUE_TAC, dateTime, 1, TEST_VALUE_CAUSE_VALUE, TEST_VALUE_SUB_CAUSE_CODE);//2 impacted subscribers for HIER3_ID_2 (TEST_VALUE_IMSI and TEST_VALUE_IMSI_1)
        insertData(TEST_VALUE_IMSI, TEST_VALUE_CATEGORY_ID, TEST_VALUE_HIER3_ID_3, TEST_VALUE_HIER321_ID_1,
                TEST_VALUE_TAC, dateTime, 1, TEST_VALUE_CAUSE_VALUE, TEST_VALUE_SUB_CAUSE_CODE);//HIER3_ID_3 not in TEST_VALUE_BSC_GROUP so not included
        insertData(TEST_VALUE_IMSI_1, TEST_VALUE_CATEGORY_ID, TEST_VALUE_HIER3_ID_1, TEST_VALUE_HIER321_ID_1,
                SAMPLE_EXCLUSIVE_TAC, dateTime, 1, TEST_VALUE_CAUSE_VALUE, TEST_VALUE_SUB_CAUSE_CODE);//exclusive tac not included
    }
    ...
    private void insertData(final String imsi, final String categoryID, final String controllerID, final String cellID,
        ...
        for (int i = 0; i < instances; i++) {
            ...
            dataForRawTable.put(CAUSE_GROUP, cause_grp);
            dataForRawTable.put(EXTENDED_CAUSE, ex_cause_grp);
    ...
    private void insertAggregatedEventData() throws Exception {
        ...
        insertAggregatedRow(TEST_VALUE_HIER3_ID_1, TEST_VALUE_CATEGORY_ID, 15, 30, dateTime, TEST_VALUE_CAUSE_VALUE,
                TEST_VALUE_SUB_CAUSE_CODE);
        insertAggregatedRow(TEST_VALUE_HIER3_ID_2, TEST_VALUE_CATEGORY_ID, 7, 14, dateTime, TEST_VALUE_CAUSE_VALUE,
                TEST_VALUE_SUB_CAUSE_CODE);
        insertAggregatedRow(TEST_VALUE_HIER3_ID_3, TEST_VALUE_CATEGORY_ID, 2, 4, dateTime, TEST_VALUE_CAUSE_VALUE,
                TEST_VALUE_SUB_CAUSE_CODE);
    }

    private void insertAggregatedRow(final String controllerID, final String categoryID, final int numberOfErrors,
            final int numberOfSuccesses, final String time, final String cause_grp, final String ex_cause_grp)
            throws SQLException {
        ...
        dataForAggTable.put(CAUSE_GROUP, cause_grp);
        dataForAggTable.put(EXTENDED_CAUSE, ex_cause_grp);
