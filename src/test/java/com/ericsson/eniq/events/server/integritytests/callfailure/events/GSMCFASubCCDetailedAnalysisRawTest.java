...
import com.ericsson.eniq.events.server.test.queryresults.gsm.callfailure.GSMDetailedEventCFACsfBySubCauseCodeResult;
import com.ericsson.eniq.events.server.test.util.DateTimeUtilities;
import com.sun.jersey.core.util.MultivaluedMapImpl;

public class GSMCFASubCCDetailedAnalysisRawTest extends
        BaseDataIntegrityTest<GSMDetailedEventCFACsfBySubCauseCodeResult> {

    private ControllerSubCCDetailedAnalysisService service;
    ...
    private static final String TEST_AF_ID_VALUE = "1";

    private static final String TEST_AF_CAUSE_VALUE = "0";

    private static final String TEST_AF_CAUSE_DESC_VALUE = "RADIO INTERFERENCE MESSAGE FAILURE";

    private static final String TEST_AF_CAUSE_SHORT_DESC_VALUE = "RADIO INTERFERENCE MESSAGE FAILURE";

    private static final String TEST_AF_CAUSE_ORIGIN_VALUE = "BSC Originated";

    private static final String AF_CAUSE_DISPLAY_VALUE = TEST_AF_CAUSE_SHORT_DESC_VALUE + ", "
            + TEST_AF_CAUSE_ORIGIN_VALUE;
    ...
    public void onSetUp() throws Exception {
        service = new ControllerSubCCDetailedAnalysisService();
    ...
    public void testRawDataFiveMinutesQuery() throws Exception {
        ...
        validateAgainstGridDefinition(jsonResult, "GSM_NETWORK_BSC_CC_SCC_CSF_DETAILED_EVENT_ANALYSIS");
    }

    private void verifyResult(final String json) throws Exception {
        final List<GSMDetailedEventCFACsfBySubCauseCodeResult> resultRecord = getTranslator().translateResult(json,
                GSMDetailedEventCFACsfBySubCauseCodeResult.class);
        assertThat(resultRecord.size(), is(NUMBER_OF_MATCHING_EVENTS));
        for (int i = 0; i < NUMBER_OF_MATCHING_EVENTS; i++) {
            final GSMDetailedEventCFACsfBySubCauseCodeResult result = resultRecord.get(i);
    ...
    private void insertRawEventData() throws SQLException {
        ...
        for (int i = 0; i < NUMBER_OF_MATCHING_EVENTS; i++) {
            ...
            valuesForCauseCodeCFATable.put(CATEGORY_ID, GSM_CALL_SETUP_FAILURE_CATEGORY_ID);
            ...
            valuesForCauseCodeCFATable.put(GSM_CSF_AF_ID, TEST_AF_ID_VALUE);
            valuesForCauseCodeCFATable.put(GSM_CSF_AF_CAUSE, TEST_AF_CAUSE_VALUE);
    ...
    private void insertLookupData() throws SQLException {
        ...
        valuesForTable.clear();
        valuesForTable.put(GSM_CSF_AF_ID, TEST_AF_ID_VALUE);
        valuesForTable.put(GSM_CSF_AF_CAUSE, TEST_AF_CAUSE_VALUE);
        valuesForTable.put(GSM_CSF_CAUSE_DESC, TEST_AF_CAUSE_DESC_VALUE);
        valuesForTable.put(GSM_CSF_SHORT_DESC, TEST_AF_CAUSE_SHORT_DESC_VALUE);
        valuesForTable.put(GSM_CSF_ORIGIN, TEST_AF_CAUSE_ORIGIN_VALUE);
