...
import com.ericsson.eniq.events.server.test.queryresults.gsm.callfailure.CFAMsisdnEventSummaryResult;
...
public class CFAMsisdnEventSummaryRawTest extends BaseDataIntegrityTest<CFAMsisdnEventSummaryResult> {

    private MSISDNSubscriberSummaryService gsmCFAMsisdnEventSummaryService;

    @Before
    public void onSetUp() throws Exception {
        gsmCFAMsisdnEventSummaryService = new MSISDNSubscriberSummaryService();
        attachDependencies(gsmCFAMsisdnEventSummaryService);
        createTables();
    ...
    public void testFiveMinuteQuery() throws URISyntaxException, Exception {
        ...
        requestParameters.add(TYPE_PARAM, TYPE_MSISDN);
        requestParameters.add(CATEGORY_ID, "0");
        requestParameters.add(MSISDN_PARAM, "12345");
        //requestParameters.add(IMSI_PARAM, "123456789123456");
        requestParameters.add(DISPLAY_PARAM, GRID_PARAM);

        final String result = runQuery(gsmCFAMsisdnEventSummaryService, requestParameters);
        verifyResult(result);
    }

    private void verifyResult(final String json) throws Exception {
        assertJSONSucceeds(json);
        final List<CFAMsisdnEventSummaryResult> results = getTranslator().translateResult(json,
                CFAMsisdnEventSummaryResult.class);

        assertThat(results.size(), is(2));
        validateAgainstGridDefinition(json, "GSM_SUBSCRIBER_SUMMARY_EVENT_ANALYSIS_BY_MSISDN");
        final CFAMsisdnEventSummaryResult csf_result = results.get(0);
        final CFAMsisdnEventSummaryResult cd_result = results.get(1);
        ...
        assertThat(cd_result.getMmisdn(), is("12345"));
    ...
    private void insertData() throws Exception {
        ...
        final Map<String, Object> dataFor_TEMP_DIM_E_IMSI_MSISDN = new HashMap<String, Object>();
        dataFor_TEMP_DIM_E_IMSI_MSISDN.put(MSISDN, "12345");
        dataFor_TEMP_DIM_E_IMSI_MSISDN.put(IMSI_PARAM, "123456789123456");

        insertRow(TEMP_DIM_E_IMSI_MSISDN, dataFor_TEMP_DIM_E_IMSI_MSISDN);
    ...
    private void createTables() throws Exception {
        ...
        final Collection<String> columnsFor_TEMP_DIM_E_IMSI_MSISDN = new ArrayList<String>();
        columnsFor_TEMP_DIM_E_IMSI_MSISDN.add(IMSI);
        columnsFor_TEMP_DIM_E_IMSI_MSISDN.add(MSISDN);
        createTemporaryTable(TEMP_DIM_E_IMSI_MSISDN, columnsFor_TEMP_DIM_E_IMSI_MSISDN);
