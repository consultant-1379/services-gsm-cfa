...
import com.ericsson.eniq.events.server.serviceprovider.impl.gsm.dataconnection.events.SubscriberSCCDetailedService;
import com.ericsson.eniq.events.server.test.queryresults.gsm.dataconnection.SubscriberDetailedEventAnalysisResult;
 ...
 *
 */
public class ImsiDetailedEventBySubCauseCodeRawTest extends
        BaseDataIntegrityTest<SubscriberDetailedEventAnalysisResult> {

    private SubscriberSCCDetailedService service;
    ...
    private static final String TEST_VALUE_TBF_RELEASE_CAUSE = "2";

    private static final String TEST_VALUE_TBF_RELEASE_CAUSE_DESC = "TBF released due to missing radio contact or removal of resources (no response from BTS after Immediate Assignment have been sent, timing advance channel faulty, no answer from the MS, essential channel removed)";
    ...
    public void setup() throws Exception {
        service = new SubscriberSCCDetailedService();
    ...
    public void testFiveMinuteQuery() throws URISyntaxException, Exception {
        ...
        requestParameters.add(SUB_CAUSE_CODE_PARAM, TEST_VALUE_CHANNEL_RELATED_RELEASE_CAUSE);
        final String result = runQuery(service, requestParameters);
        verifyResult(result, TEST_VALUE_GSM_CELL1_NAME, TEST_VALUE_TAC, TEST_VALUE_MANUFACTURER,
                TEST_VALUE_MARKETING_NAME, 6/*we expect 4 because we also want the entry with exclusive TAC counted*/);
    }
    ...
    public void testLeftOuterJoins() throws URISyntaxException, Exception {
        ...
        requestParameters.add(IMSI_PARAM, TEST_VALUE_IMSI1);
        requestParameters.add(TYPE_PARAM, TYPE_IMSI);
        requestParameters.add(SUB_CAUSE_CODE_PARAM, TEST_VALUE_CHANNEL_RELATED_RELEASE_CAUSE_GROUP_NO_LOOKUP);
        final String result = runQuery(service, requestParameters);

        verifyLeftOuterJoinValues(result);
    }

    private void verifyResult(final String json, final String expectedCellName, final int expectedTAC,
            final String expectedManufacturer, final String expectedMarketingName, final int expectedResultCount)
            throws Exception {
        assertJSONSucceeds(json);
        validateAgainstGridDefinition(json, "GSM_PS_SUBSCRIBER_DETAILED_EVENT_ANALYSIS_BY_IMSI");
        final List<SubscriberDetailedEventAnalysisResult> results = getTranslator().translateResult(json,
                SubscriberDetailedEventAnalysisResult.class);

        assertThat(results.size(), is(expectedResultCount));
        final SubscriberDetailedEventAnalysisResult result = results.get(0);
        ...
        assertThat(results.get(3).getAccessArea(), is(TEST_VALUE_GSM_CELL2_NAME));
    }
    ...
    private void verifyLeftOuterJoinValues(final String json) throws Exception {
        ...
        final SubscriberDetailedEventAnalysisResult result = results.get(0);
        assertThat(result.getEventType(), is(TEST_VALUE_CATEGORY_ID_DESC));
        assertThat(result.getAccessArea(), is(""));
        assertThat(result.getController(), is(""));
        assertThat(result.getTac(), is(TEST_VALUE_TAC_NO_LOOKUP));
        assertThat(result.getTerminalMake(), is(""));
        assertThat(result.getTerminalModel(), is(""));
        assertThat(result.getTbfReleaseCause(), is(TEST_VALUE_TBF_RELEASE_CAUSE_DESC));
        assertThat(result.getTbfDataVolume(), is(TEST_VALUE_TBF_DATA_VOLUME));
        assertThat(result.getTbfDuration(), is(TEST_VALUE_TBF_DURATION));
        assertThat(result.getTerminalMake(), is(""));
        assertThat(result.getTerminalModel(), is(""));
        assertThat(result.getGprsMeasReportRxqualDlDesc(), is(""));
        assertThat(result.getGprsMeasReportCvalueDesc(), is(""));
        assertThat(result.getGprsMeasReportCvBepDesc(), is(""));
        assertThat(result.getGprsMeasReportMeanBepDesc(), is(""));
        assertThat(result.getGprsMeasReportSignVarDesc(), is(""));
        assertThat(result.getChannelRelatedReleaseCauseGroupDesc(), is(""));
        assertThat(result.getAccessArea(), is(""));
        assertThat(result.getController(), is(""));
    }
    ...
    private void insertLookupData() throws SQLException {
        ...
        valuesForTable.clear();
        valuesForTable.put(HIERARCHY_3, TEST_VALUE_GSM_CONTROLLER1_NAME);
        valuesForTable.put(HIER321_ID, TEST_VALUE_HIER321_ID2);
        valuesForTable.put(HIERARCHY_1, TEST_VALUE_GSM_CELL2_NAME);
        valuesForTable.put(HIER3_ID, TEST_VALUE_HIER3_ID);
        valuesForTable.put(VENDOR, ERICSSON);
        valuesForTable.put(RAT, "0");
        insertRow(TEMP_DIM_E_SGEH_HIER321, valuesForTable);
