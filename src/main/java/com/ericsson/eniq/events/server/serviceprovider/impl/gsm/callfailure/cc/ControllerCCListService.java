...
@Stateless
@Local(Service.class)
public class ControllerCCListService extends GenericService {
    ...
    public String getTemplatePath() {
        return GSM_CALL_FAILURE_CONTROLLER_CAUSE_CODE_LIST_ANALYSIS;
    }
    ...
    public Map<String, QueryParameter> getServiceSpecificQueryParameters(
        ...
        if (node != null) {
            ...
            long hashBscId = 0;
            if (allData.length == 3) {
                hashBscId = getQueryUtils().createHashIDForController(allData[2], allData[0], allData[1]);
                if (hashBscId != 0) {
                    queryParameters.put(CONTROLLER_SQL_ID, QueryParameter.createLongParameter(hashBscId));
                }
            }
        } else {
            final String bscGroupName = requestParameters.getFirst(GROUP_NAME_PARAM);
            if (bscGroupName != null) {
                queryParameters.put(GROUP_NAME_PARAM, QueryParameter.createStringParameter(bscGroupName));
            }
        }
    ...
    public AggregationTableInfo getAggregationView(final String type) {
        return new AggregationTableInfo("HIER3_CG", EventDataSourceType.AGGREGATED_15MIN,
