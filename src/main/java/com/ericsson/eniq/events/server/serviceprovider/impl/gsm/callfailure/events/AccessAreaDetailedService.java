...
@Stateless
@Local(Service.class)
public class AccessAreaDetailedService extends GenericService {
    ...
    public String getTemplatePath() {
        return GSM_CALL_FAILURE_ACCESS_AREA_DETAILED_EVENT_ANALYSIS;
    }
        ...
        if (hashedIdForCellAsString == null) {
            ...
            if (node != null) {
                ...
                if (cellGroupName != null) {
                    queryParameters.put(GROUP_NAME_KEY, QueryParameter.createStringParameter(cellGroupName));
                    requestParameters.remove(GROUP_NAME_PARAM);
                } else {
                    final String[] nodeParameter = node.split(DELIMITER);
                    final long hashedIdForCell = getQueryUtils().createHashIDForCell(nodeParameter[4],
                            nodeParameter[2], nodeParameter[1], nodeParameter[0], nodeParameter[3]);
                    queryParameters.put(CELL_SQL_ID, QueryParameter.createLongParameter(hashedIdForCell));

                }
            }
        } else {
            final long hashedIdForCell = Long.parseLong(hashedIdForCellAsString);
            queryParameters.put(CELL_SQL_ID, QueryParameter.createLongParameter(hashedIdForCell));
